...
### Normal Distribution
A bell-shaped curve. Most of the data are clustered around the center: the median and mean are in the center of the curve. The distribution is symmetric meaning about half of the data is to the left of the mean, while the other half is to the right of the mean. The mean and median are approximately the same
...
### Left skew 
The data are clustered around a large value and mostly fall to the right of the mean (including the median).  It's also referred to as negative skewed. 
...
### Right skew  
The data are clustered around a small value and most of the data (including the median) lie to the left of the mean. 
...
The data is equally distributed: there are no clear peaks in the data. 
...
After getting a brief overview of our data set, now we want to know, what's the most common life expectancy?  

To answer this question, let's print the distribution of life expectancies in the data set. 
...
But, we also notice the shape of this histogram is bimodal. So, this tells us that we're likely looking at two different groups (or more). 
  
  
  
#### Divide the groups, separate the data  
Since we suspect we're looking at multiple groups from the bimodal distribution, let's separate the histrograms by one of the variables in our data set, continent.
...
You can manually hard code each of the variables to the colors you want.  
...
The below is code to produce a density plot.  Notice the y-axis is labelled "density", not "count" like the histogram plots. 
...
The below code shows you how to choose custom colors for each continent and split each plot. 
```{r}
...
## Practial implications for Survey Analysis  
So, how can knowing all this code help you?  
...
Ok, we heard you. There's a ramp up to R. If you're still uncomfortable, you can view histogram examples in google sheets [here](https://docs.google.com/spreadsheets/d/1KC1Cx1nHfNrVRSom1QXc42q8S_VSo9KmJA9nrZx3Ils/edit?usp=sharing) and view their walkthrough on the Google support page [here](https://support.google.com/docs/answer/9146867?hl=en)
