...
But, we also notice the shape of this histogram is bimodal. So, this tells us that we're likely looking at two different groups. 
...
#create a seperate data tabe calculating the mean and median of the data set
measures_of_ct_by_country <- gapminder %>% 
  group_by(continent) %>% 
...
ggplot(gapminder, aes(lifeExp, fill = continent)) +
  geom_histogram() + 
  geom_vline(data = measures_of_ct_by_country, 
             aes(xintercept = mean, color = "mean"), 
             linetype = "dashed") +
  geom_vline(data = measures_of_ct_by_country, 
             aes(xintercept = median, color = "median"), 
...
Now we can see that Africa and Europe have two very distinct means of life expectancy.   
...
```{r, warning = FALSE}
ggplot(gapminder, 
       aes(lifeExp, stat(density), colour = continent)) +
  geom_freqpoly()
```
