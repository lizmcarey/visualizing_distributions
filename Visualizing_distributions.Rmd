...
When you see a bimodal distribution, consider breaking your data set into differnt groups to separate the peaks.   
...
From the printed table, we see teh data set containes the life expectancy, population, and GDP per capita of various countries separated by year.  
...
## Density Plots  
A density plot is a smoothed version of the histogram. It uses kernal smoothing to plot values so it removed the "noise" from a typical histogram visualization.  
...
Let's label the nonrespondents in the `sample` data set in a column named `respondent_type`.
...
Let's calculate the average tenure and lifetime trips of the respondents and nonrespondents 
```{r}
...
From this visualisation, we can see the respondents and nonrespondents are likely similar groups.  
